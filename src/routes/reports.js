...
router.get('/sales-summary', authenticate, authorize(['admin','moderador','gerente']), async (req, res) => {
  try {
    ...
    const row = await knex('sales')
      ...
      .select(
        ...
        knex.raw('COALESCE(SUM(total_usd_cents * tasa_usd_registrada_micro),0) as total_weighted_micro'),
...
router.get('/sales-detail', authenticate, authorize(['admin','moderador','gerente']), async (req, res) => {
  try {
    const { startDate, endDate } = req.query || {};
    if (!startDate || !endDate) return res.status(400).json({ error: 'startDate and endDate are required (YYYY-MM-DD)' });
